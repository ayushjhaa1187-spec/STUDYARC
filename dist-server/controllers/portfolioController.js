...
export const submitPortfolio = async (req, res) => {
    try {
        ...
        const evidenceData = [];
        if (githubUrl)
            evidenceData.push({ project_id: project.id, evidence_type: 'github', url: githubUrl });
        if (liveDemoUrl)
            evidenceData.push({ project_id: project.id, evidence_type: 'live_demo', url: liveDemoUrl });
