 ...
 */
export async function getTopMentors(criteria: MentorMatchCriteria) {
    ...
    // E.g.: return await supabase.rpc('match_mentors', { p_tags: requiredTags, p_budget: budget, p_limit: limit });
    ...
    const scoredMentors = mentors.map((m: any) => {
        // Calculate intersection of requested tags and mentor tags
        const mentorTags = m.expertise_tags || [];
        const intersection = mentorTags.filter((tag: string) => requiredTags.includes(tag)).length;
